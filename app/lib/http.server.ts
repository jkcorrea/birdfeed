...
export const response = {
  ...
  error: (cause: unknown, options: ResponseOptions) => {
    ...
    return json(makeErrorPayload(reason), makeOptions({ status: reason.status, ...options }))
  },
  ...
  deferError: (cause: unknown, options: ResponseOptions) => {
    ...
    return defer(makeErrorPayload(reason), makeOptions({ status: reason.status, ...options }))
  },
  redirect: (url: string, options: ResponseOptions) => redirect(url, makeOptions(options)),
}
