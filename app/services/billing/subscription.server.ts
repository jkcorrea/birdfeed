...
import { Currency, SubscriptionStatus, TierId } from '@prisma/client'
...
export async function createSubscription({
  ...
  currentPeriodStart,
  currentPeriodEnd,
}: Pick<Subscription, 'id' | 'tierId' | 'priceId' | 'itemId' | 'currentPeriodStart' | 'currentPeriodEnd'> &
...
export async function updateSubscription({
  ...
  Pick<
    Subscription,
    'tierId' | 'status' | 'priceId' | 'itemId' | 'currentPeriodStart' | 'currentPeriodEnd' | 'cancelAtPeriodEnd'
  >) {
  try {
...
export async function deleteSubscription({ id, customerId }: Pick<User, 'customerId'> & Pick<Subscription, 'id'>) {
  try {
